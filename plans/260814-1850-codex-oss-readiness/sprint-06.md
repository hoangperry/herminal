...
**Status:** complete for automatable scope — issues #3/#5/#6/#7 closed; owner gates remain
...
1. [x] Issue #3 — add synthetic Codex direct/package-manager wrapper fixtures,
   malformed/unknown negatives, and classification tests without real argv.
2. [x] Issue #5 — add a source-level diagnostics privacy guard with allowed and
   forbidden fixtures; remove current sensitive metadata logging; run it in CI.
3. [x] Issue #6 — migrate `Package.swift` from `swiftLanguageVersions` to
   `swiftLanguageModes` without changing Swift 6 or macOS 14 requirements.
4. [x] Issue #7 — add a sub-800-word lifecycle-signal architecture tour with real
   source/symbol links, privacy/MainActor boundaries, and fixture-test location.
5. [x] Re-run full CI (153 tests, run `31822247206`) and CodeQL (run
   `31822247125`), review the diff, then close only issues whose
